import { Image, Table, Tag, Card, Divider, Input, Avatar, Skeleton} from 'antd';
...
const BeLateUsers = () => {
    ...
    useEffect(()=>{
        axiosApiInstance.get(url.BaseUrl+url.userBeLate).
        then((response)=>{
          setLoad(false);
        ...
        axiosApiInstance.get(url.BaseUrl + url.analistic).then((res)=>{
...
<Card>
  ...
  <Card
            title="Жүйеден кешігушіні табу"
    bordered={false}

  >
    <Divider orientation="left">Кешігушіні табу <SearchOutlined /></Divider>

    <Search placeholder="Адам атын немесе ЖСН ді жазыңыз" onSearch={
      (value)=>{
        axiosApiInstance.get(`${url.BaseUrl}${url.BeLateBooks}?search=${value}`)
        .then((response)=>{
